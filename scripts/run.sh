...
echo -e "\n===== 4xx ve 5xx HTTP Hata Kodları =====" >> $RAPOR
...
echo -e "\n===== NGINX Servis Logları (journalctl -u nginx) =====" >> $RAPOR

journalctl -u nginx --since "1 hour ago" | tail -n 50 >> $RAPOR

echo -e "\n===== Sistem Hata Kayıtları (journalctl -p 3) =====" >> $RAPOR
journalctl -p 3 -xb | tail -n 30 >> $RAPOR

echo -e "\nRapor oluşturuldu: $(realpath $RAPOR)"
